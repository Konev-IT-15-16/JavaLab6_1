# Решетникова_IT15_Лабораторная работа 6
## Вариант 4.
# Задание 1
Аннотации
## Задача 1
...
## Алгоритм решения
Разработана аннотация @Invoke для автоматического вызова помеченных методов. 
Класс InvokeReflection с методом showInvoke() использует рефлексию Java: принимает 
объект, анализирует его класс, находит все методы с аннотацией @Invoke и выполняет их. 
Для обеспечения доступа к приватным методам используется setAccessible(true), вызов 
осуществляется через Method.invoke(). Исключения перехватываются с выводом сообщений, 
что гарантирует устойчивость программы.
...
Reflection API также помогает создавать новые экземпляры классов, вызывать методы и получать или
устанавливать значения полей.
