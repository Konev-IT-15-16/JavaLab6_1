...
* Реализован обработчик ToStringReflection:
 * Анализирует поля класса
 * Исключает поля с Mode.ON
 * Формирует строку формата: <code>Person [ name = Иван age = 20 role = Стажёр ] </code>
* Используется setAccessible(true) для приватных полей
